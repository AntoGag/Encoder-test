...
public class Autonomous_Encoder_Test extends LinearOpMode {
    ...
    static final double DRIVE_GEAR_REDUCTION = 2.0; //Gear Reduction
    ...
    public void runOpMode() throws InterruptedException {
        ...
        if (opModeIsActive()) { //Checks if opMode is still active
            ...
            FrontLeftTarget = FrontLeft.getCurrentPosition() + (int) (60 * COUNTS_PER_INCH); //Set target position for encoder
            FrontRightTarget = FrontRight.getCurrentPosition() + (int) (60 * COUNTS_PER_INCH); //Set target position for encoder
            BackLeftTarget = BackLeft.getCurrentPosition() + (int) (60 * COUNTS_PER_INCH); //Set target position for encoder
            BackRightTarget = BackRight.getCurrentPosition() + (int) (60 * COUNTS_PER_INCH); //Set target position for encoder

            FrontLeft.setTargetPosition(FrontLeftTarget); //Set motor to target position
            FrontRight.setTargetPosition(FrontRightTarget); //Set motor to target position
            BackLeft.setTargetPosition(BackLeftTarget); //Set motor to target position
            BackRight.setTargetPosition(BackRightTarget); //Set motor to target position
            ...
            while (opModeIsActive() && (runtime.seconds() < 10) && (FrontLeft.isBusy() && FrontRight.isBusy() && BackLeft.isBusy() && BackRight.isBusy())) { //Checks whether opMode is still active, Runtime is not greater than timeout time, and motors are busy.
